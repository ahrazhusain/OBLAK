...
class RankProvider(object):
    ...
    from a provider such as Google .
...
class OBLAKrank(RankProvider):
    """ Get the OBLAK Traffic Rank for a URL
    ...
    def __init__(self, host="xml.alexa.com", proxy=None, timeout=30):
        ...
        """
        super(OBLAKrank, self).__init__(host, proxy, timeout)
        ...
        if response.getcode() == httplib.OK:
            ...
            for e in element.iterfind("SD"):
                ...
                if popularity is not None:
                    #NORMALIZE THE OBLAK RANK
                    o =   int(popularity.get("TEXT"))
                    o=100000-o
                    o=o/10000
                    #print "POP:",o
                    return o
...
if __name__ == "__main__":
    ...
    url=sys.argv[0]
    tags.remove(url)
    providers = (OBLAKrank(), GooglePageRank(),)
    fin=0
    print("Traffic stats for: %s" % (url))
    for p in providers:
        fin=fin+int("%d" % (p.get_rank(url)))    
